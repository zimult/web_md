    ...
    try:
        ...
        for bn in banners:
            ...
            for module in modules:
                ...
                cursor.execute(
                    "SELECT id FROM resource_banner where `position`=%d and resource_id=%d" % (module, banner))
                result = cursor.fetchone()
                if result is None:
                    t = time.time()
                    ts = int(round(t * 1000))
...
def post_to_search(article_id, title, description, h5, author):
    ...
    dict = {"resourceId": str(article_id), "title": title, "description": description, "h5url": h5, "publisher": author,
            "type": "0"}
...
def is_vip_category(cursor, article_id):
    ...
    for row in result:
        list.append(row[0])
    nsync_module_name = u"预览"
    if nsync_module_name in list:
        return True
    return False
