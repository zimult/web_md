var tasks = new Vue({
    ...
    data: {
        ...
        step: {0: "文件过大", 1:"等待tag", 2:"等待文案", 3:"程序错误"}
    },
    methods: {
...
$(document).ready(function(){
    ...
    loadFailData(function(params, data, error){
        waitingDialog.hide()
        if (error != null) {
            alert(error)
        } else {
            ...
            for (var i = data.result.length - 1; i >= 0; i--) {
                ...
                image = {}
                image['guid'] = re['guid']
                image['store_url'] = re['store_url']
                image['textinfo'] = re['textinfo']
                image['fsize'] = re['fsize'] / 1000
                if(re.hasOwnProperty("lossy_fsize")){
                    image['fsize'] = re['lossy_fsize'] / 1000
                }
                image['step'] = tasks.step[re['step']]

                image['selected'] = false
                tasks.task_images.push(image)
            }
        }
    })
...
////////////ACTION////////////////////////////////////////////////////////////////
function loadFailData(callback) {
    var params = {
    };
    var url = DOMAIN+"/wx/listnottowx";
    //提交
