...
function updateMatch() {
    ...
    if (mid == '' || title.length == 0 || date == '' || city.length == 0 ) {
       alert("请完成输入数据");
       return;
    }
    update_match(mid, title, date, city, sd, gc, url,live_url)
};

function update_match(mid, title, date, city, sd, gc, url, live_url) {
	waitingDialog.show("查询中...");
	_update_match(mid, title, date, city, sd, gc, url, live_url, function(params, data, error){
        waitingDialog.hide()
        if (error) {	
            alert(error)
        } else {
...
function _update_match(mid, title, date, city, sd, gc, z_url, l_url, callback)	{
    var params = {
        "id": mid,
        "title": title,
        "date": date,
